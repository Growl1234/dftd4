...
subroutine weight_references_hessian(self, mol, cn, q, gwvec, gwdcn, gwd2cn)
   ...
   !> Weighting for the atomic reference systems and its CN derivatives
   ...
   !$omp private(iat, izp, iref, igw, norm, dnorm, d2norm, gw, expw, expd, expdd, &
   ...
   do iat = 1, mol%nat
      ...
      do iref = 1, self%ref(izp)
         ...
         gwvec(iref, iat, 1) = gwk*zscale
         gwdcn(iref, iat, 1) = dgwk*zscale
         gwd2cn(iref, iat, 1) = d2gwk*zscale
      end do
   end do
...
subroutine get_atomic_c6_hessian(self, mol, gwvec, gwdcn, gwd2cn, c6, &
   ...
   do ipair = 1, npair
      ...
      do iref = 1, self%ref(izp)
         do jref = 1, self%ref(jzp)
            refc6 = self%c6(iref, jref, izp, jzp)
            c6ij = c6ij + gwvec(iref, iat, 1)*gwvec(jref, jat, 1)*refc6
            dc6dcni = dc6dcni + gwdcn(iref, iat, 1)*gwvec(jref, jat, 1)*refc6
            dc6dcnj = dc6dcnj + gwvec(iref, iat, 1)*gwdcn(jref, jat, 1)*refc6
            d2c6dcni = d2c6dcni + gwd2cn(iref, iat, 1)*gwvec(jref, jat, 1)*refc6
            d2c6dcnj = d2c6dcnj + gwvec(iref, iat, 1)*gwd2cn(jref, jat, 1)*refc6
            d2c6mix = d2c6mix + gwdcn(iref, iat, 1)*gwdcn(jref, jat, 1)*refc6
         end do
      end do
      c6(iat, jat) = c6ij
      c6(jat, iat) = c6ij
